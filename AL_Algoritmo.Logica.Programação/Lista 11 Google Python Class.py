...
def donuts(n):
  if n >= 10:
    string = 'Número de donuts: muitos'
  else:
    string = f'Número de donuts: {n}'
  return string
...
def fixa_primeiro(s):
  letra = s[0]
  fixa_primeiro = letra + s[1:].replace(letra, '*')
  return fixa_primeiro
